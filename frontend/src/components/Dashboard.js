    ...
    const fetchData = async () => {
      ...
      const attemptFetch = async () => {
        ...
        if (selectedPeriod === 'current') {
          ...
          const response = await fetch(`${process.env.REACT_APP_API_URL}${process.env.API_PREFIX ? `/${process.env.API_PREFIX}` : ''}/dashboard?${queryParams}`, {
            headers: {
        ...
        if (contentType?.includes('text/html') || responseText.toLowerCase().includes('<!doctype')) {
          ...
          console.log('URL da requisição:', `${process.env.REACT_APP_API_URL}${process.env.API_PREFIX ? `/${process.env.API_PREFIX}` : ''}/dashboard?${queryParams}`);
    ...
    const fetchAllTransactions = async () => {
      ...
      const attemptFetch = async () => {
        ...
        if (!token) {
          ...
          const response = await fetch(`${process.env.REACT_APP_API_URL}${process.env.API_PREFIX ? `/${process.env.API_PREFIX}` : ''}/dashboard/all-transactions`, {
            headers: {
            'Authorization': `Bearer ${token}`
          }
        });
