...
const Login = () => {
  ...
  const fetchBanks = async (retryCount = 0, delay = 1000) => {
    try {
      ...
      const apiUrl = `${process.env.REACT_APP_API_URL}${process.env.API_PREFIX ? `/${process.env.API_PREFIX}` : ''}/banks`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (step === 'email') {
        ...
        const response = await fetch(`${process.env.REACT_APP_API_URL}${process.env.API_PREFIX ? `/${process.env.API_PREFIX}` : ''}/auth/check-email`, {
          method: 'POST',
        ...
        try {
          ...
          const response = await fetch(`${process.env.REACT_APP_API_URL}${process.env.API_PREFIX ? `/${process.env.API_PREFIX}` : ''}/auth/send-code`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        try {
          ...
          const response = await fetch(`${process.env.REACT_APP_API_URL}${process.env.API_PREFIX ? `/${process.env.API_PREFIX}` : ''}/auth/verify-code`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
  ...
  const handleTelegramLink = async () => {
    try {
      const verificationResponse = await fetch(`${process.env.REACT_APP_API_URL}${process.env.API_PREFIX ? `/${process.env.API_PREFIX}` : ''}/telegram/init-verification`, {
        method: 'POST',
        headers: {
