...
const Expenses = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const categoriesPromise = fetch(`${process.env.REACT_APP_API_URL}/${process.env.API_PREFIX}/categories`, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
        
        const banksPromise = fetch(`${process.env.REACT_APP_API_URL}/${process.env.API_PREFIX}/banks/favorites`, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
  ...
  const fetchExpenses = async () => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}/${process.env.API_PREFIX}/expenses?${queryParams}`, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
  ...
  const handleDelete = async (expense) => {
    try {
      ...
      if (deleteOptions.type === 'bulk') {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/${process.env.API_PREFIX}/expenses/bulk`, {
          method: 'DELETE',
          headers: {
      ...
      if (expense.is_recurring) {
        ...
        url = `${process.env.REACT_APP_API_URL}/${process.env.API_PREFIX}/expenses/${expense.id}/recurring`;
        ...
        if (deleteOption === 'all') {
          url = `${process.env.REACT_APP_API_URL}/${process.env.API_PREFIX}/expenses/${expense.id}?delete_all_installments=true`;
        } else {
          url = `${process.env.REACT_APP_API_URL}/${process.env.API_PREFIX}/expenses/${expense.id}`;
        }
      }
      else {
  ...
  const handleSave = async (expenseData) => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}/${process.env.API_PREFIX}/expenses/${editingExpense ? editingExpense.id : ''}`, {
  ...
  const handleDeleteConfirm = async (option) => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}/${process.env.API_PREFIX}/expenses/${expenseToDelete.id}/recurring`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${auth.token}`
  ...
  useEffect(() => {
    const loadExpenseToEdit = async () => {
      if (id) {
        try {
          const response = await fetch(`${process.env.REACT_APP_API_URL}/${process.env.API_PREFIX}/expenses/${id}`, {
            headers: {
              'Authorization': `Bearer ${auth.token}`
            }
          });
