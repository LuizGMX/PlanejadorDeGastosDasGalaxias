...
const Incomes = () => {
    ...
    const fetchData = async () => {
      try {
        const [categoriesResponse, banksResponse] = await Promise.all([
          fetch(`${process.env.REACT_APP_API_URL}/incomes/categories`, {
            headers: {
  ...
  const fetchIncomes = async () => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}/incomes?${queryParams}`, {
        headers: {
          'Authorization': `Bearer ${auth.token}`
        }
      });
  ...
  const handleDelete = async (income) => {
    try {
      ...
      if (deleteOptions.type === 'bulk') {
        ...
        const response = await fetch(`${process.env.REACT_APP_API_URL}/incomes/bulk`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${auth.token}`,
      ...
      let url = `${process.env.REACT_APP_API_URL}/incomes/${incomeId}`;
  ...
  const handleSave = async (incomeData) => {
    try {
      ...
      const url = `${process.env.REACT_APP_API_URL}/incomes${isUpdate ? `/${incomeData.id}` : ''}`;
